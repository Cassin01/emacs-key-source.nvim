...
local function _win_open(row, height, col)
  ...
  local win = va.nvim_open_win(buf, true, {col = col, row = (row - 5), relative = "editor", anchor = "NW", style = "minimal", height = height, width = math.floor(((vim.o.columns / 2) - 2)), focusable = false, border = "rounded"})
